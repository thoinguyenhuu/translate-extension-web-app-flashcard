...
function normalizeVocabularyItem(item) {
  ...
  return { word, pos, mainMeaning, meanings };
}

async function fetchVocabulary() {
  const { data, error } = await supabase
    .from("vocabulary")
    .select("word, pos, meaning, main_meaning, meanings")
    .order("created_at", { ascending: false });

  if (error) {
    throw new Error(`Unable to load vocabulary (${error.message}).`);
  }

  const normalizedRows = (data || []).map(normalizeVocabularyItem).filter(Boolean);
...
async function deleteVocabularyWord(word) {
  ...
  if (error) {
    throw new Error(`Unable to delete "${word}" (${error.message}).`);
  }
}
...
function pickWeightedWord(words, learnedMap, difficultyMap, previousWord = "") {
  ...
  const weightedWords = activeWords.map((item) => {
    ...
    if (activeWords.length > 1 && item.word === previousWord) weight *= 0.35;
    return { item, weight };
  });
  ...
  for (const entry of weightedWords) {
    ...
    if (randomValue <= 0) return entry.item;
  }
...
function createStudyRound(words, learnedMap, difficultyMap, previousWord = "") {
  ...
  if (promptType === "quiz") {
    ...
    if (question) {
      return { type: "quiz", word: nextWord.word, question };
    }
  }

  return { type: "flashcard", word: nextWord.word };
}
...
function AppContent() {
  ...
  function goToNextRound(nextLearnedMap = learnedMap, nextDifficultyMap = difficultyMap) {
    const nextRound = createStudyRound(
      vocabulary, nextLearnedMap, nextDifficultyMap, currentRound?.word || ""
    );
  ...
  function handleFlip() {
    if (!currentCard || currentRound?.type !== "flashcard") return;
    setIsFlipped((value) => !value);
  }

  function handleAnswerSelect(answer) {
    if (!currentRound?.question || selectedAnswer) return;
  ...
  async function handleDeleteConfirm() {
    if (!confirmingWord || deletingWord) return;
  ...
  function handleSetDifficulty(level) {
    ...
    const nextDifficultyMap = { ...difficultyMap, [currentCard.word]: level };
    setDifficultyMap(nextDifficultyMap);
  ...
  function handleRemembered() {
    ...
    const nextLearnedMap = { ...learnedMap, [currentCard.word]: true };
    setLearnedMap(nextLearnedMap);
